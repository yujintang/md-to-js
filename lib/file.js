...
module.exports = {
    ...
    writeJs: function (file, allArr) {
        ...
        for (let i of allArr) {
            ...
            || ${everyParamT}`;
                ...
                if (regExp.test(forMust[j])) {  //false
                    if (forData[j]) {
                        everyParamD = `let ${forData[j]} = params.${forData[j]} || new ${forType[j]};     // 必填: ${forMust[j]};  含义: ${forName[j]}`;
                    }
                } else {  //true
                    if (forData[j]) {
                        everyParamD = `let ${forData[j]} = params.${forData[j]};     // 必填: ${forMust[j]};  含义: ${forName[j]}`;
                    }
                }
        ...
        ${everyParamD} `;
            ...
            let file_mock = path.resolve(process.cwd(), './mock/');
            if (!fs.existsSync(file_mock)) {
                fs.mkdirSync(file_mock);
            }
            let mockUrl = path.join(file_mock, i.mockUrl);
            mockUrl = path.resolve(mockUrl);

            fs.writeFile(mockUrl, utils.mock_template(i.mockDate), function (err, data) {
                if(err) {
                    console.log('错误!  --->  ' + chalk.red(i.name + '  接口url格式错误'));
                    process.exit(1);
                }
    ...
    createMd: function (url) {
        ...
        if (!/[\w\W]+\.md$/.test(url)) {
            console.log('错误!  --->  ' + chalk.red(url + '  格式错误,必须为 .md 的文件'));
            process.exit(1);
        }
        ...
        fs.writeFile(url, utils.md_template(), function (err) {
            if (err) {
                ...
                console.log('MD文件生成成功!  --->  ' + chalk.magenta(url));
